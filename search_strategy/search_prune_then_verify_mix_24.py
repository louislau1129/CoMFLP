...
if __name__ == "__main__":
    ...
    setup_logger(log_name=log_name, save_dir='log_24')
    ...
    if args.num_layers == 24:
        if args.select_measure == "dc" or args.select_measure == "rand":
            ...
            dc_mat = np.load(f"../dump_24/dc_mat_{args.mode}_bs{args.bs}_iter{args.iter}.npy")    
        elif args.select_measure == "svcca":
            dc_mat = np.load(f"../dump_24/cca_mat_{args.bs * args.iter}{args.svcca_mode}_svthre{args.thre}.npy") 
        elif args.select_measure == "cosine":
    ...
    if args.search_mode == "exhaustive":
        ...
        if args.select_measure != "rand":
            ...
            regulated_clipped_layers_list = [[[1,12]], [[2,13]], [[6, 17]], [[7,18]], [[12,23]], [[13,24]]]
            regulated_clipped_layers_list = [[[1,18]], [[2,19]], [[4, 21]], [[6,23]], [[7,24]]]
            regulated_clipped_layers_list = [[[1,12]], [[6,17]], [[13,24]]]
            regulated_clipped_layers_list = [[[1,18]], [[3,20]], [[7,24]]]
            top_measure = []
    ...
    pretrained_path_name = "qinyue/wav2vec2-large-xlsr-53-chinese-zn-cn-aishell1"
    ...
    if args.num_layers == 24:
        ...
        model = Wav2Vec2ForCTC(config=config)
        pt_path = "../exp/clip24to12_simple_ft/clip24to12.pt" 
    ...
    if args.search_mode == "exhaustive":
        ...
        for i, idx in enumerate(topk_idx):
            ...
            clipped_model = Wav2Vec2ForCTC(config=new_config)
        ...
        WERS = []
        LastLayerDCs = []
        for i in range(np.abs(args.beam_size)):
